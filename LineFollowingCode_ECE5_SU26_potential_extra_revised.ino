...
void ReadPhotoResistors() {
  for (int i = 0; i < totalPhotoResistors; i++) { 
    ...
    LDR[i] = constrain(LDR[i], 0, 100); //constrains values between 0 and 100.
  }    
...
void CalcError() {
  ...
  // Only update error if result is valid
    if (isfinite(WeightedAve)) {
...
void PID_Turn() {
  ...
  if (!isfinite(error) || !isfinite(lasterror) || !isfinite(sumerror)) { //recover and if the pid returns NAN
    error = 0;
    lasterror = 0;
    sumerror = 0;
  }
