...
void RunMotors() {
  M1SpeedtoMotor = min(max(NOMINALSPEED + SpRead + M1P,-100), 255); 
  M2SpeedtoMotor = min(max(NOMINALSPEED + SpRead + M2P,-100), 255); // remember M1Sp & M2Sp is defined at beginning of code (default 60)
...
void runMotorAtSpeed(side _side, int speed) {
  if (_side == LEFT) {
    if (speed > 0)         {       // swap direction if speed is negative
      ledcWrite(M1L, speed);
      ledcWrite(M1H, 0);          // sets the direction of the motor from arguments
  } else {
      ledcWrite(M1L, 0);
      ledcWrite(M1H, abs(speed)); // sets the direction of the motor from arguments
  }
  }
  if (_side == RIGHT) {
    if (speed > 0)  {              // swap direction if speed is negative
      ledcWrite(M2L, speed);
      ledcWrite(M2H, 0); // sets the direction of the motor from arguments
  } else {
      ledcWrite(M2L, 0);
      ledcWrite(M2H, abs(speed)); // sets the direction of the motor from arguments
  }
  }
}
